...
d = pd.read_excel(r'.\InputData.xlsx')
d=d.fillna('')
...
for l in range(1,L):
    ...
    W.append(np.random.rand(N[l]*N[l-1]).reshape(N[l],N[l-1])*np.sqrt(1/N[l]))
...
for i in range(0,num_itr):
    ...
    for l in range(1,L):
        if l!=1:
            D.append(np.random.rand(A[l-1].shape[0],A[l-1].shape[1])<keep_prob[l])
            A[l-1]*= D[l-1]
            A[l-1]/=keep_prob[l]
